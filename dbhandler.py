...
class dbHandler:
  ...
  def sql_do(self, sql, params = ()):
    """
        ...
        method for non-select queries
          sql is a string containing SQL
          params is a list containing parameters
        returns nothing
    """
  ...
  def update_nocommit(self, recid, rec):
    """
        ...
        update a row in the table
          ...
          rec is a dict with key/value paris corresponding to table schema
    """
    klist = sorted(rec.keys())
    values = [rec[k] for k in klist]
     
    for i, k in enumerate(klist): # don't update id field
      if k == 'id':
        del klist[i]
        del values[i]
  ...
  def _filename(self, fn):
    ...
    # set the row factory to the built in row factory, it allows rows to be read in several wasy (dict/tuple/list/row/etc)
...
def test():
    ...
    db = dbHandler(filename=fn, table=t)
    print('Done.')
